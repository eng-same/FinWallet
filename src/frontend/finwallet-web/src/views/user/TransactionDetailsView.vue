...
const loadDetails = async (quiet = false) => {
  if (!quiet) loading.value = true;
  try {
    txDetails.value = await transactionStore.fetchTransactionDetails(txId);
  } catch (error: any) {
  ...
  <q-page class="q-pa-lg flex flex-center">
    <div class="w-full max-w-2xl">
      ...
      <div class="row items-center justify-between q-mb-md">
        ...
        >
          <q-tooltip>{{ $t('adminTx.refreshBtn') }}</q-tooltip>
        </q-btn>
      </div>
      ...
      <q-card class="glass-card q-pa-lg" v-if="txDetails">
        <q-card-section class="q-pa-none">
          ...
          <!-- Receipt Link CTA -->
          <div class="row justify-center q-mb-lg border-bottom q-pb-lg" v-if="txDetails.receipt">
            <q-btn
              color="secondary"
              icon="receipt"
              label="View Digital Receipt"
              no-caps
              class="rounded-borders text-bold hover-scale"
              @click="router.push(`/transactions/${txId}/receipt`)"
            />
          </div>
